...
import {
  View, Text, TextInput, Pressable, StyleSheet, ActivityIndicator, ScrollView, Modal,
} from 'react-native';
...
export function Screen({ children, scroll = false, style, edges = ['top'], refreshControl }) {
  ...
  const extra = scroll
    ? { contentContainerStyle: [{ padding: space[4], paddingBottom: space[8] }, style], refreshControl, showsVerticalScrollIndicator: false, keyboardShouldPersistTaps: 'handled' }
    : { style: [{ flex: 1 }, style] };
  return (
...
export const SectionTitle = ({ children, right }) => (
  <View style={s.sectionTitle}>
    <Text style={[font.tiny, { color: colors.textMuted, textTransform: 'uppercase' }]}>{children}</Text>
    {right}
  </View>
);
...
export function Card({ children, style, onPress, accent }) {
  const body = (
    <View
      style={[
        s.card,
        accent ? { borderColor: tint(accent, 0.35), backgroundColor: tint(accent, 0.06) } : null,
        style,
      ]}
    >
      {children}
...
export function Btn({ title, onPress, variant = 'primary', icon, loading, disabled, style, small }) {
  const v = {
    ...
    secondary: { bg: 'rgba(255,255,255,0.06)', fg: colors.text, border: colors.border },
    ghost: { bg: 'transparent', fg: colors.textSub, border: 'transparent' },
    danger: { bg: colors.dangerDim, fg: colors.danger, border: tint(colors.danger, 0.3) },
  }[variant];
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        s.btn,
        small && { paddingVertical: space[2], paddingHorizontal: space[3] },
    ...
    >
      {loading ? (
        ...
        <>
          {icon ? <Ionicons name={icon} size={small ? 15 : 17} color={v.fg} /> : null}
...
export const IconBtn = ({ icon, onPress, color = colors.textSub, size = 20, style }) => (
  <Pressable
    hitSlop={10}
    onPress={onPress}
    style={({ pressed }) => [s.iconBtn, pressed && { opacity: 0.6 }, style]}
  >
  ...
  return (
    <View style={[{ marginBottom: space[3] }, style]}>
      ...
      <TextInput
        ...
        style={[s.input, multiline && { minHeight: 110, textAlignVertical: 'top', paddingTop: space[3] }]}
        multiline={multiline}
        {...props}
      />
...
export function Chip({ label, active, onPress, color = colors.primary, icon }) {
  return (
    <Pressable
      onPress={onPress}
      style={[
        s.chip,
        active
          ? { backgroundColor: tint(color, 0.16), borderColor: tint(color, 0.5) }
      ...
      {items.map((it) => {
        ...
        return (
          <Pressable
            ...
            style={[s.segItem, active && { backgroundColor: colors.primaryDim, borderColor: tint(colors.primary, 0.35) }]}
          >
            <Text style={[font.small, { color: active ? colors.primary : colors.textSub, fontWeight: '700' }]}>
              {it.label}
            </Text>
          </Pressable>
        );
...
export function SwitchRow({ label, hint, value, onChange, icon, color = colors.primary }) {
  return (
    <Pressable onPress={() => onChange(!value)} style={s.switchRow}>
      {icon ? (
        <View style={[s.switchIcon, { backgroundColor: tint(color, 0.12) }]}>
...
export function Badge({ label, color = colors.primary }) {
  return (
    <View style={[s.badge, { backgroundColor: tint(color, 0.14), borderColor: tint(color, 0.32) }]}>
      <Text style={[font.tiny, { color }]}>{label}</Text>
    </View>
  );
}
...
export function Empty({ icon = 'sparkles-outline', title, hint }) {
  return (
    <View style={s.empty}>
      ...
      {hint ? (
        <Text style={[font.small, { color: colors.textMuted, textAlign: 'center', marginTop: space[2] }]}>{hint}</Text>
      ) : null}
    </View>
  );
}
...
export function Banner({ type = 'info', message, onClose }) {
  ...
  const c = { info: colors.cyan, success: colors.primary, error: colors.danger }[type];
  return (
    <View style={[s.banner, { backgroundColor: tint(c, 0.12), borderColor: tint(c, 0.35) }]}>
      <Ionicons
        ...
        color={c}
      />
      <Text style={[font.small, { color: c, flex: 1 }]}>{message}</Text>
      {onClose ? <IconBtn icon="close" size={15} color={c} onPress={onClose} /> : null}
    </View>
  );
}

/** Bottom sheet đơn giản dựng trên Modal — dùng cho form nhanh. */
...
const s = StyleSheet.create({
  ...
  segmented: {
    flexDirection: 'row', gap: space[1], padding: space[1],
    backgroundColor: 'rgba(255,255,255,0.04)', borderRadius: radius.sm,
    borderWidth: 1, borderColor: colors.border,
  },
  segItem: {
    flex: 1, alignItems: 'center', paddingVertical: space[2],
    borderRadius: radius.sm - 3, borderWidth: 1, borderColor: 'transparent',
  },
  switchRow: {
    flexDirection: 'row', alignItems: 'center', gap: space[3], paddingVertical: space[3],
  },
  switchIcon: { width: 34, height: 34, borderRadius: radius.sm, alignItems: 'center', justifyContent: 'center' },
  track: {
    width: 46, height: 27, borderRadius: radius.pill, padding: 3,
    backgroundColor: 'rgba(255,255,255,0.08)', justifyContent: 'center',
  },
  knob: { width: 21, height: 21, borderRadius: radius.pill, backgroundColor: colors.textMuted },
  badge: { paddingHorizontal: space[2], paddingVertical: 3, borderRadius: radius.pill, borderWidth: 1 },
  empty: { alignItems: 'center', justifyContent: 'center', paddingVertical: space[8], paddingHorizontal: space[5] },
  emptyIcon: {
